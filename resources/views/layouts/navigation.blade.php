<nav x-data="{ open: false, darkMode: localStorage.getItem('theme') === 'dark' }" class="bg-white dark:bg-gray-800 border-b border-gray-100 dark:border-gray-700">
        ...
        <div class="flex justify-between h-16">
            ...
            <div class="hidden sm:flex sm:items-center sm:ms-6 gap-2">
                <button @click="darkMode = !darkMode; if(darkMode){ document.documentElement.classList.add('dark'); localStorage.setItem('theme', 'dark'); } else { document.documentElement.classList.remove('dark'); localStorage.setItem('theme', 'light'); }"
                    :class="{'bg-gray-200': !darkMode, 'bg-gray-700': darkMode}"
                    class="p-2 rounded focus:outline-none transition-colors duration-200"
                    x-init="if(darkMode){ document.documentElement.classList.add('dark'); } else { document.documentElement.classList.remove('dark'); }">
                    <span x-show="!darkMode" class="text-gray-800">🌞</span>
                    <span x-show="darkMode" class="text-yellow-300">🌙</span>
                </button>
